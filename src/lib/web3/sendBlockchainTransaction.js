...
function sendBlockchainTransaction(provider, fromOrPlainPrivateKey, to, data, value) {
    ...
    return new Promise(async(ok, ko) => {
        try {
            ...
            tx.gasPrice = web3.utils.toHex(web3.utils.toWei(await gasCalculator(provider), 'gwei'));
            tx.gasLimit = web3.utils.toHex((await sendAsync(provider, 'eth_getBlockByNumber', 'latest', false)).gasLimit);
            ...
            tx.gasLimit = window.bypassEstimation ? tx.gasLimit : web3.utils.toHex(web3.utils.toBN(parseInt(parseInt(await sendAsync(provider, 'eth_estimateGas', tx)) * 1.10269)).toString());
            window.bypassEstimation && await sendAsync(provider, 'eth_estimateGas', tx);
            var sendTransaction;
            if (privateKey) {
